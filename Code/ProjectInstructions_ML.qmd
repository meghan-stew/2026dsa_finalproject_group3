...
#corn_prep <- corn_recipe %>%
  #testing just the training split
 # prep(training = corn_train)

#corn_prep

#baked_train <- bake(corn_prep, new_data = NULL)
...
```
```{r test untuned model}

#xgb_test_spec <- boost_tree(
 # trees = 300,
 # tree_depth = 4,
#  min_n = 10,
 # learn_rate = 0.05
...
#xgb_test_fit <- xgb_test_spec %>%
#  fit(yield_mg_ha ~ ., data = baked_train)


```


```{r test anova race latin hypercube }
#set.seed(235)
#small_folds <- vfold_cv(corn_train, v = 5, strata = yield_mg_ha)

#set.seed(432)
#small_grid <- grid_space_filling(
...
```{r test workflow with ANOVA}
#xgb_wf_test <- workflow() %>%
  #add_recipe(corn_recipe) %>%
  #add_model(xgb_spec)

#test small anova chunk below for troubleshooting
...
```{r final workflow and xgboost-specification}
...
#workflow allows us to load xgboost into other test qmd. 
...
#added verbose function because of runtime issues with this chunk. 
