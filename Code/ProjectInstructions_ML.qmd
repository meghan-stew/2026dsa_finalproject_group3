...
```{r defining year as character for when needed}
 data_model <- data_model %>%
  mutate(
...
weather_features <- weather_daily %>%
  ...
  # making new variables by taking sums and means where appropriate below: 
  group_by(site, year) %>%
  summarise(
...
corn_recipe <- recipe(yield_mg_ha ~ ., data = corn_train) %>%
  ...
  step_novel(all_nominal_predictors()) %>% #handles novel/unseen factors
  step_unknown(all_nominal_predictors()) %>% #used for handling NA values if they appear
  step_dummy(all_nominal_predictors()) %>% #used to convert nominal data into binary for ML 
...
#making workflow first to keep using model and recipe together
...
##Parallel Processing and Metric Exploration
...
#using the same split as for XG Boost: remember to re-run before XG BOOST section

gam_corn_recipe <- recipe(yield_mg_ha ~ ., data = corn_train) %>%
  step_string2factor(all_nominal_predictors())%>%
  step_other(hybrid, threhold = 0.01) %>%
  step_unknown(all_nominal_predictors ()) %>%
  step_novel(all_nominal_predictors())

#similar to xgboost recipe setup 
...
gam_corn_spec <- gen_additive_mod() %>%
  set_engine("mgcv") %>%
  set_mode("regression")
...
#gam_corn_wf <- workflow() %>%
  #add_recipe(gam_corn_recipe) %>%
 # add_model(
  #  gam_corn_spec,
 #   formula = yield_mg_ha ~ 
  #    s(total_prcp) +
   # s(prcp_apr_sep) +
    ...
    #s(mean_vp) +
    #s(mean_dayl) +
    #  site + year + hybrid + previous_crop
 # )


#this didn't work but the attempt has been left for your review. 
```


```{r workaround code for recipe }
library(mgcv)
...
#used this instead of recipe that didn't work. 
#feature note: hybrid was removed from this final model training chunk because it was not running with this factor included. 
...
```{r GAM model fitting}
#don't use this unless workaround chunk above doesn't work. 

#gam_fit <- fit(gam_corn_wf, data = corn_train)

```

```{r GAM train predictions}

# GAM: predicting on training data:
corn_train_preds <- tibble( .pred = predict(gam_fit, newdata = corn_train), 
yield_mg_ha = corn_train$yield_mg_ha 
...
```{r train metrics}
#Viewing TRAIN GAM Metrics 

metric_set(rmse, rsq)(corn_train_preds, truth = yield_mg_ha, estimate = .pred)


```

```{r test set  }
corn_test_preds <- tibble( .pred = predict(gam_fit, newdata = corn_test), 
yield_mg_ha = corn_test$yield_mg_ha 
...
```{r test metrics}
#Viewing TEST GAM Metrics 

metric_set(rmse, rsq)(corn_test_preds, truth = yield_mg_ha, estimate = .pred)


```
```{r exporting worfklow}
saveRDS(gam_fit, "../Data/gam_fit.rds")

```

#Sources For Project:
1. https://www.rdocumentation.org/packages/mgcv/versions/1.9-4/topics/predict.gam 
2. https://rdrr.io/cran/mgcv/man/missing.data.html
3. https://www.r-bloggers.com/2021/03/how-to-solve-common-problems-with-gams/
4. https://nrennie.rbind.io/blog/forecasting-gams-questions/
5. https://stat.ethz.ch/R-manual/R-devel/library/mgcv/help/gam.html
6. https://m-clark.github.io/generalized-additive-models/application.html
7. https://www.geeksforgeeks.org/r-language/generalized-additive-models-using-r/
8. https://stat.ethz.ch/R-manual/R-devel/RHOME/library/mgcv/html/gam.selection.html
9. https://dplyr.tidyverse.org/reference/rename.html
10. https://www.datanovia.com/en/lessons/rename-data-frame-columns-in-r/
11. https://readr.tidyverse.org/reference/write_delim.html
12. https://stackoverflow.com/questions/40120179/saving-data-frame-to-csv-with-column-names-as-strings-in-r
13. https://workflows.tidymodels.org/

AI LLM model used for building shinyapp: Claude 4.0 
AI used for troubleshooting code: perplexity best model and google notebook LM referencing class notes
